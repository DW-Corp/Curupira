...
impl MockHttpClient {
    ...
    async fn get(&self, url: &str) -> MockResponse {
        ...
        match url {
            ...
            u if u.contains("/.well-known/openid_configuration") => {
            ...
            u if u.contains("/.well-known/jwks.json") => {
                MockResponse::json(json!({"keys": []}))
            }
...
async fn test_openid_discovery_endpoint() {
    ...
    let client = MockHttpClient::new();
    let response = client.get(&format!("{}/.well-known/openid_configuration", TEST_SERVER_URL)).await;
...
async fn test_jwks_endpoint() {
    ...
    let client = MockHttpClient::new();
    let response = client.get(&format!("{}/.well-known/jwks.json", TEST_SERVER_URL)).await;
...
async fn test_token_endpoint_unauthorized() {
    ...
    let response = client.post(&format!("{}/auth/token", TEST_SERVER_URL), body).await;
...
async fn test_userinfo_endpoint_unauthorized() {
    ...
    let client = MockHttpClient::new();
    let response = client.get(&format!("{}/auth/userinfo", TEST_SERVER_URL)).await;
...
async fn test_introspect_endpoint_unauthorized() {
    ...
    let response = client.post(&format!("{}/auth/introspect", TEST_SERVER_URL), body).await;
...
async fn test_registration_endpoint_bad_request() {
    ...
    let response = client.post(&format!("{}/register", TEST_SERVER_URL), body).await;
...
async fn test_login_endpoint_bad_request() {
    ...
    let response = client.post(&format!("{}/login", TEST_SERVER_URL), body).await;
...
async fn test_nonexistent_endpoint() {
    ...
    let client = MockHttpClient::new();
    let response = client.get(&format!("{}/nonexistent", TEST_SERVER_URL)).await;
...
async fn test_error_handling_patterns() {
    ...
    for (endpoint, expected_status) in error_scenarios {
        let response = if endpoint.starts_with("/auth/token") || endpoint.starts_with("/auth/introspect") {
            client.post(&format!("{}{}", TEST_SERVER_URL, endpoint), "").await
        } else {
            client.get(&format!("{}{}", TEST_SERVER_URL, endpoint)).await
        };
...
async fn test_complete_oauth_discovery_flow() {
    ...
    // Step 1: Discovery
    let discovery_response = client.get(&format!("{}/.well-known/openid_configuration", TEST_SERVER_URL)).await;
    assert_eq!(discovery_response.status(), 200);
    let discovery = discovery_response.parse_json().unwrap();
    
    // Step 2: JWKS
    let jwks_response = client.get(&format!("{}/.well-known/jwks.json", TEST_SERVER_URL)).await;
    assert_eq!(jwks_response.status(), 200);
    let jwks = jwks_response.parse_json().unwrap();
    assert!(jwks["keys"].is_array());
    
    // Step 3: Authorization endpoint accessibility
    let auth_response = client.get(&discovery["authorization_endpoint"].as_str().unwrap()).await;
